...
impl Solution {
    pub fn decrypt(code: Vec<i32>, k: i32) -> Vec<i32> {
        match k.cmp(&0) {
            ...
            _ => {
                let mut ans = vec![];
                for x in 0..code.len() {
                    ans.push(Solution::code_i_k_sum(&code, x as i32, k));
                }
                ans
            }
        }
    ...
    pub fn code_i_k_sum(code: &Vec<i32>, i: i32, k: i32) -> i32 {
        let mut sum = 0;
        let mut idx = 0;
        for j in 1..=k.abs() {
            if k > 0 {
                if i + j < code.len() as i32 {
                    idx = i + j;
                } else {
                    idx = i + j - code.len() as i32;
                }
            } else {
                if i - j >= 0 {
                    idx = i - j;
                } else {
                    idx = code.len() as i32 + i - j;
                }
            }
